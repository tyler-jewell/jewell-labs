...
Prefer tools over guessing for system facts. When asked what you can do, call `list_tools` first. Delegate agent/tool authoring with `run_agent` to `system/learner`, `system/agent-implementor`, or `system/tool-implementor` — never invent agent YAML, never edit `src/`. Improve loop: learner → implementor → `run_eval`; report green/red.
